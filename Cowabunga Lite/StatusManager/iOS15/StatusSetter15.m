...
- (void) applyChanges:(StatusBarOverrideData*)overrides {
    FILE *outfile;
    NSURL *workspace = [[DataSingleton shared] getCurrentWorkspace];
    NSURL *path = [workspace URLByAppendingPathComponent:@"StatusBar/HomeDomain/Library/SpringBoard/statusBarOverrides"];
    NSString *pathstr = [path path];
    const char *cPath = [pathstr UTF8String];
...
- (StatusBarOverrideData*) getOverrides {
    ...
    if ([fileManager fileExistsAtPath:pathstr]) {
        ...
        StatusBarOverrideData* overrides = [data mutableBytes];
        infile = fopen (cPath, "r");
        if (infile == NULL) return NULL;
        fread(overrides, sizeof(StatusBarOverrideData), 1, infile);
        fclose (infile);
        return overrides;
    } else {
        NSMutableData* data = [NSMutableData dataWithLength:sizeof(StatusBarOverrideData)];
